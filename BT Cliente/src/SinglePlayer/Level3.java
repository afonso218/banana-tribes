...
public class Level3 extends JLabel implements Level, Runnable {
	...
	public void paint(Graphics g) {
		...
		if(!startPress){
			...
			g.drawString("LEVEL 3", 400-100, 400);
		}
	}
	...
	public boolean isReiAlive(){
		if(mapa.getP1().getRei().getLife() <= 0 || mapa.getP2().getRei().getLife() <= 0){
			System.out.println("1 REI DOWN");
			if(mapa.getP1().getRei().getLife() <= 0)
				mapa.ReiMorre(mapa.getP1().getID());
			if(mapa.getP2().getRei().getLife() <= 0)
				mapa.ReiMorre(mapa.getP2().getID());
			System.out.println("REI MOREE");
			return false;
		}
		return true;
	}
